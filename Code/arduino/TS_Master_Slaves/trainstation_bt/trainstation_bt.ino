...
*/
// Versione script: 1.5.1 - BT MASTER-SLAVE + TIPI TRENO
...
// BT Slave: comunica con display LCD remoto
// Tipi treno: 1=FB, 2=FR, 3=I, 4=IC, 5=RV, 6=ICN, 7=R
...
// === SCRIPT VERSION ===
const char scriptVersion[] = "1.5.1-BT-SLAVE-TRAIN";
...
// === BLUETOOTH ===
BluetoothSerial SerialBT;  // BT per comandi da cellulare
BluetoothSerial SerialBT_Slave;  // BT per comunicare con slave
bool btEnabled = false;  // BT cellulare attivo
bool btSlaveEnabled = false;  // BT slave attivo
...
// === METEO VARIABILI ===
// Valori di default (in attesa di GPS collegato)
...
void printHelp(bool useBT=false){
  ...
  if (useBT) {
    ...
    s->println(F("    X=tipo (1-7): 1=FB,2=FR,3=I,4=IC,5=RV,6=ICN,7=R"));
    s->println(F("    Y=binario (1-9), Z=azione (1=part,2=arr)"));
  ...
  s->printf("  BT Cellulare: %s", btEnabled ? "ATTIVO" : "OFF");
  if (btEnabled) {
    s->println(F(" [LED BLU]"));
  ...
  s->printf("  BT Slave: %s", btSlaveEnabled ? "ATTIVO" : "OFF");
  if (btSlaveEnabled) {
    s->print(F(" ("));
    s->print(SerialBT_Slave.connected() ? F("connesso") : F("in attesa"));
    s->println(F(")"));
  } else {
    s->println();
  }
  ...
  s->println(F("\n  === TIPI TRENO (per playtrain) ==="));
  s->println(F("  1=FB  Frecciabianca"));
  s->println(F("  2=FR  Frecciarossa"));
  s->println(F("  3=I   Interregionale"));
  s->println(F("  4=IC  Intercity"));
  s->println(F("  5=RV  Regionale Veloce"));
  s->println(F("  6=ICN Intercity Notte"));
  s->println(F("  7=R   Regionale"));
  s->println(F("---\n"));
}
...
// === BLUETOOTH SLAVE HELPER ===
bool initSlaveBlueooth() {
  if (btEnabled) {
    Serial.println(F("⚠️  BT cellulare attivo, slave BT disabilitato"));
    return false;
  }
  
  if (btSlaveEnabled) {
    Serial.println(F("BT slave già attivo"));
    return true;
  }
  
  Serial.println(F("🔵 Attivo BT Slave..."));
  if (!SerialBT_Slave.begin("M9Lab-TrainStation-Master")) {
    Serial.println(F("❌ Impossibile avviare BT Slave"));
    return false;
  }
  
  btSlaveEnabled = true;
  Serial.println(F("✅ BT Slave attivo: M9Lab-TrainStation-Master"));
  return true;
}

void stopSlaveBluetooth() {
  if (!btSlaveEnabled) return;
  
  Serial.println(F("Disattivo BT Slave..."));
  SerialBT_Slave.flush();
  SerialBT_Slave.end();
  delay(100);
  btSlaveEnabled = false;
  Serial.println(F("BT Slave disattivato"));
}

void sendToSlave(const char* command) {
  if (!btSlaveEnabled) {
    Serial.println(F("⚠️  BT Slave non attivo, comando non inviato"));
    return;
  }
  
  if (SerialBT_Slave.connected()) {
    SerialBT_Slave.println(command);
    Serial.print(F("📤 Inviato a slave: "));
    Serial.println(command);
  } else {
    Serial.println(F("⚠️  Slave non connesso"));
  }
}

void sendTrainToSlave(String dest, String bin, String linea1, String linea2, String orario, String tipo) {
  char cmd[128];
...
void toggleBluetooth() {
  if (btEnabled) {
    ...
    // RIATTIVA BT SLAVE dopo aver spento BT cellulare
    delay(500);
    if (!btSlaveEnabled) {
      Serial.println(F("Riattivo BT Slave..."));
      initSlaveBlueooth();
    }
    ...
    // CONFLITTO: disabilita BT slave prima di attivare BT cellulare
    if (btSlaveEnabled) {
      Serial.println(F("⚠️  Disabilito BT Slave per attivare BT cellulare"));
      stopSlaveBluetooth();
      delay(300);
    }
...
void processCommand(char* cmd, bool fromBT=false) {
  ...
  // Comandi AUDIO - BLOCCATI se BT attivo
    ...
    if (fromBT) {
      ...
      // Parsing comando: es "141" = train 1, binario 4, azione 1
      ...
      if (strlen(trainCmd) >= 3) {
        ...
        // Nome treno sempre MEZZANINELAB
        String trainName = "MEZZANINELAB";
        
        // Genera codice treno: prefisso + numero random
        String trainPrefix;
        switch(train) {
          case 1: trainPrefix = "FB"; break;  // Frecciabianca
          case 2: trainPrefix = "FR"; break;  // Frecciarossa
          case 3: trainPrefix = "I"; break;   // Interregionale
          case 4: trainPrefix = "IC"; break;  // Intercity
          case 5: trainPrefix = "RV"; break;  // Regionale veloce
          case 6: trainPrefix = "ICN"; break; // Intercity notte
          case 7: trainPrefix = "R"; break;   // Regionale
          default: trainPrefix = "R"; break;
        }
        
        // Genera numero treno random (4 cifre)
        ...
        s->print(binario);
        s->print(F(" "));
        s->println(tipoOrario);
        
        // Invia allo slave
        sendTrainToSlave("MF-TRIESTE", String(binario), trainName, trainCode, orarioStr, tipoOrario);
      }
    ...
    if (fromBT) {
      ...
      sendToSlave("METEO");  // Invia anche allo slave
...
void setup() {
  ...
  if (ntpSuccess) {
    Serial.println(F("✅ Orario sincronizzato"));
    
    // Recupera nome città da coordinate GPS (reverse geocoding)
    if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println(F("Annuncio meteo..."));
  ...
  // === INIZIALIZZA BT SLAVE PER COMUNICAZIONE CON DISPLAY ===
  Serial.println(F("\n--- Inizializzazione BT Slave ---"));
  if (initSlaveBlueooth()) {
    Serial.println(F("✅ BT Slave pronto per connessione display"));
  } else {
    Serial.println(F("⚠️  BT Slave non disponibile"));
  }
  Serial.println(F("---\n"));
}
...
void loop() {
  ...
  if (randomPlayFlag && !btEnabled && !playingPlaylist && millis() - lastRandomEvent > randomInterval) {
    ...
    if (tipo == 0) {
      // Alert random - slave non fa nulla
      sendToSlave("ALERT");
      ...
      // Annuncio treno random - invia a slave
      int train = random(1, 8);  // 1-7 tipi di treno
      int binario = random(1, 10);
      ...
      // Invia comando allo slave
      sendTrainToSlave("MF-TRIESTE", String(binario), trainName, trainCode, orarioStr, tipoOrario);
      
      executeAudioPlayList(cmd);
      playPlaylist();
    } else {
      // Annuncio meteo random - invia a slave
