...
# Carica background per calcolare dimensioni reali del green screen
if os.path.exists(WINDOW_TEMPLATE_PATH):
    bg_temp = cv2.imread(WINDOW_TEMPLATE_PATH)
    bg_h, bg_w = bg_temp.shape[:2]
    
    # Calcola dimensioni reali del green screen in pixel (distanza tra punti)
    gs_width_px = int((GS_TOP_RIGHT_X - GS_TOP_LEFT_X) * bg_w)
    gs_height_px = int((GS_BOTTOM_LEFT_Y - GS_TOP_LEFT_Y) * bg_h)
    target_ratio = gs_width_px / gs_height_px
    
    print(f"Background: {bg_w}x{bg_h}")
    print(f"Green screen: {gs_width_px}x{gs_height_px} (ratio {target_ratio:.2f})")
else:
    # Nessun background, usa ratio dello sfondo impostato
    target_ratio = PHOTO_W / PHOTO_H
    print(f"Nessun background, uso ratio: {target_ratio:.2f}")

# Calcola quale dimensione usare per il crop al ratio del green screen
current_ratio = actual_w / actual_h

if current_ratio > target_ratio:
    # Webcam più larga del green screen, calcola larghezza target
    CROP_HEIGHT = actual_h
    CROP_WIDTH = int(actual_h * target_ratio)
else:
    # Webcam più alta del green screen, calcola altezza target
    CROP_WIDTH = actual_w
    CROP_HEIGHT = int(actual_w / target_ratio)

print(f"Webcam: {actual_w}x{actual_h} (ratio {current_ratio:.2f})")
print(f"Crop webcam: {CROP_WIDTH}x{CROP_HEIGHT} (ratio {CROP_WIDTH/CROP_HEIGHT:.2f})")
print(f"Output finale: {PHOTO_W}x{PHOTO_H}")
...
def update_preview():
    if mode == MODE_PREVIEW:
        ret, frame = cap.read()
        if ret:
            # Crop al ratio del green screen
            frame_h, frame_w = frame.shape[:2]
            offset_x = (frame_w - CROP_WIDTH) // 2
            offset_y = (frame_h - CROP_HEIGHT) // 2
            frame_cropped = frame[offset_y:offset_y+CROP_HEIGHT, offset_x:offset_x+CROP_WIDTH]
            
            # Ridimensiona preview al formato dello sfondo
            frame_resized = cv2.resize(frame_cropped, (PREVIEW_W, PREVIEW_H), interpolation=cv2.INTER_LINEAR)
...
def capture_photo():
    ...
    # ================= CROP FOTO AL RATIO DEL GREEN SCREEN =================
    frame_h, frame_w = frame.shape[:2]
    offset_x = (frame_w - CROP_WIDTH) // 2
    offset_y = (frame_h - CROP_HEIGHT) // 2
    frame = frame[offset_y:offset_y+CROP_HEIGHT, offset_x:offset_x+CROP_WIDTH]
    
    # Salva foto croppata ad alta risoluzione
...
def show_result(image_pil):
    ...
    # Mostra foto nell'anteprima esistente
